...
class InterpreterBF{
    public:
        ...
        string getFilePath(){
            char result[512];
            return string (result, GetModuleFileName(NULL, result, 512));
        }
        ...
        int execute(){
            ...
            if (index == -1){
                cout << "\n Error when looking for the WINDOWS folder. Consider reinstalling?\n" << endl;
                usage();
                return 1;
            }
            ...
            while (run){
                ...
                switch (command){
                    case '>':
                    {
                        ComboCommand *c = combo.at(codeIndex);
